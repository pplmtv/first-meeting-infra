...
resource "aws_route53_record" "db_cname" {
  ...
  ttl     = 300
  records = [
    data.terraform_remote_state.db_firstme.outputs.db_instance_this_address
  ]
}
...
resource "aws_route53_record" "cache_cname" {
  ...
  ttl     = 300
  records = [
    data.terraform_remote_state.cache_firstme.outputs.elasticache_replication_group_this_primary_endpoint_address
  ]
}
*/